...
def create_model():
    ...
    model.compile(optimizer=Adam(learning_rate=0.0001), loss='binary_crossentropy', metrics=['accuracy', f1, precision, recall])
...
for fold_num, (train_index, val_index) in enumerate(kfold.split(X_train)):
    ...
    # Evaluasi model pada data validasi dari fold ini
    evaluation = model.evaluate(X_val_fold, y_val_fold)
    print(f"Evaluation on Fold {fold_num+1}: Loss - {evaluation[0]}, Accuracy - {evaluation[1]}")
...
test_f1 = evaluation_results[2]
test_precision = evaluation_results[3]
test_recall = evaluation_results[4]

print(f"\nEvaluation on Test Data: Loss - {test_loss}, Accuracy - {test_accuracy}, F1 - {test_f1}, Precision - {test_precision}, Recall - {test_recall}")
